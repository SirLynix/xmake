...
function main(outputdir, vsinfo)
        ...
        for _, arch in ipairs(vsinfo.archs) do
            ...
            for _, target in ipairs(project.ordertargets()) do
                ...
                if not _target then
                    ...
                    if targetdir then
                        _target.targetdir = path.relative(targetdir, _target.vcxprojdir)
                    end

                    targets[targetname] = _target
                end
                ...
                _target.sourcefiles = table.join(_target.sourcefiles or {}, (target:sourcefiles()))
                _target.headerfiles = table.join(_target.headerfiles or {}, (target:headerfiles()))
                _target.extrafiles = table.join(_target.extrafiles or {}, (target:extrafiles()))
                _target.filegroups = table.join(_target.filegroups or {}, target:get("filegroups"))
    ...
    for _, target in pairs(targets) do
        ...
        target.sourcefiles = table.unique(target.sourcefiles)
        target.headerfiles = table.unique(target.headerfiles)
        target.extrafiles = table.unique(target.extrafiles)
        target.filegroups = table.unique(target.filegroups)

        -- sort them to stabilize generation
        table.sort(target.sourcefiles)
        table.sort(target.headerfiles)
        table.sort(target.extrafiles)
        table.sort(target.filegroups)
    ...
    -- add xmake targets
    --[[for _, projectname in ipairs({"BuildAll", "Check"}) do
        local targetinfo = {}
        for mode_idx, mode in ipairs(vsinfo.modes) do
            for arch_idx, arch in ipairs(vsinfo.archs) do
                local info = {
                    mode = mode, 
                    arch = vsutils.vsarch(arch),
                    objectdir = path.join(vsinfo.solution_dir, "Unused"),
                    targetdir = path.join(vsinfo.solution_dir, "Unused")
                }
                if projectname == "BuildAll" then
                    info.buildcommand = "\"%$(XMakeExecutableFull)\" l private.plugins.xmakevs build \"%$(XMakeConfig)\""
                    info.rebuildcommand = "\"%$(XMakeExecutableFull)\" l private.plugins.xmakevs rebuild \"%$(XMakeConfig)\""
                    info.cleancommand = "\"%$(XMakeExecutableFull)\" l private.plugins.xmakevs clean \"%$(XMakeConfig)\""
                elseif projectname == "Check" then
                    info.buildcommand = "\"%$(XMakeExecutableFull)\" l private.plugins.xmakevs check"
                    info.rebuildcommand = "\"%$(XMakeExecutableFull)\" l private.plugins.xmakevs check"
                    info.cleancommand = nil
                end
                table.insert(targetinfo, info)
            end
        end

        targets["_" .. projectname] = {
            name = projectname,
            project_dir = path.join(vsinfo.solution_dir, "_XMake"),
            info = targetinfo
        }
    end]]
    ...
    local targetnames = {}
    for targetname, targetinfo in table.orderpairs(targets) do
       ...
       -- we need to set startup project for default or binary target
        ...
        if targetinfo.default == true then
            default_target = targetinfo
        elseif not default_target and targetinfo.kind == "binary" then
            default_target = targetinfo
        else
            table.insert(ordered_targets, targetinfo)
        end
    end
    if not default_target then
        table.insert(ordered_targets, 1, default_target)
    end
